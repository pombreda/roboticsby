...
VOID GetGPUInformation(DWORD* vendorId, DWORD* deviceId, DWORD* vsVersionMax, DWORD* psVersionMax,  WCHAR* vendorDescriptionString, WCHAR* deviceDescriptionString) 
{
	...
	hModule = ::LoadLibrary(L"d3d9.dll");

	if (hModule != NULL)
	{
		pDirect3DCreate9 method = (pDirect3DCreate9)::GetProcAddress((HMODULE)hModule,"Direct3DCreate9");
		if (method != NULL)
		{
			pD3D = ((*method)(D3D_SDK_VERSION));
			if (pD3D != NULL)
			{
				D3DADAPTER_IDENTIFIER9 adapter;
				ZeroMemory(&adapter, sizeof(D3DADAPTER_IDENTIFIER9));

				HRESULT hr;
				if (SUCCEEDED(hr = pD3D->GetAdapterIdentifier(NULL, NULL, &adapter)))
				{
					*vendorId = adapter.VendorId;
					*deviceId = adapter.DeviceId;
					size_t convertedChars = 0;
					mbstowcs_s(&convertedChars, deviceDescriptionString, 512, adapter.Description, 512);
					for (INT i = 0; i<PCI_VENTABLE_LEN;i++)
					{
						if (PciVenTable[i].VenId == adapter.VendorId)
						{
							mbstowcs_s(&convertedChars, vendorDescriptionString, 512, PciVenTable[i].VenFull, 512);
							break;
						}
					}
				}
				...
				ZeroMemory(&capabilities, sizeof(D3DCAPS9));	
				
				if (SUCCEEDED(hr = pD3D->GetDeviceCaps(NULL, D3DDEVTYPE_HAL, &capabilities)))
				{
					*vsVersionMax = (DWORD)((capabilities.VertexShaderVersion & 0x0000FF00) >> 8);
					*psVersionMax = (DWORD)((capabilities.PixelShaderVersion & 0x0000FF00) >> 8);
				}
			}
		}
	}
